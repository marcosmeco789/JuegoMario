...
public class PantallaMenu implements Screen {
    ...
    public void render(float delta) {
        ...
        // Dibuja el texto de las opciones, centrado
        for (int i = 0; i < numOpciones; i++) {
            ...
            // Cambia la fórmula de la posición Y para colocar la opción superior (Jugar) primero
            float y = (height - totalHeight) / 2f + (numOpciones - i - 1) * optionHeight + optionHeight / 2f;

            // Si la opción es seleccionada o tocada, la cambia a amarillo
            if (i == seleccion) {
                font.setColor(1, 1, 0, 1); // Amarillo para la opción seleccionada
            } else {
                font.setColor(0, 0, 0, 1); // Blanco para las otras opciones
            }
        ...
        if (Gdx.input.isTouched()) {
            ...
            for (int i = 0; i < opciones.length; i++) {
                ...
                float top = (height - totalHeight) / 2f + (numOpciones - i - 1) * optionHeight + optionHeight / 2f + glyphLayout.height / 2 - 10;
    ...
    private void manejarSeleccion() {
        ...
        switch (seleccion) {
            case 0: // Jugar
                game.setScreen(new PantallaJugar(game)); // Ir a PantallaJugar
                break;
            case 1: // Opciones
                game.setScreen(new PantallaOpciones(game)); // Ir a PantallaOpciones
                break;
            case 2: // Records
